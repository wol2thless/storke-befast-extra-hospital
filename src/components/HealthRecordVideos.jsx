...
// ข้อมูลวีดีโอสำหรับ HealthRecord
const HEALTH_VIDEO_CATEGORIES = [
  {
    id: "stroke_education",
  ...
  },
  {
    ...
    description: "การออกกำลังกายและกายภาพบำบัดสำหรับผู้ป่วยโรคหลอดเลือดสมอง",
    videos: [
      { id: 100, title: "กายภาพบำบัด สำหรับผู้ป่วยโรคหลอดเลือดสมอง : [ชุดความรู้ STROKE-05]", url: "https://www.youtube.com/watch?v=eEmeOHis4bE" },
...
const HealthRecordVideos = () => {
  ...
  const filteredCategories = HEALTH_VIDEO_CATEGORIES.map(category => ({
    ...category,
  ...
  return (
    <div className="bg-base-100 rounded shadow p-4 mb-6">
      <div className="text-center mb-4">
        <h2 className="text-lg font-bold text-primary mb-2">
          📚 วิดีโอความรู้สุขภาพ
        </h2>
        <p className="text-gray-600 text-sm">
          วิดีโอความรู้เกี่ยวกับโรคหลอดเลือดสมองและการดูแลสุขภาพ
        </p>
      </div>
      ...
      {selectedCategory && (
        <div className="mb-4">
          ...
          <p className="text-gray-600 mb-3 text-sm">{selectedCategory.description}</p>
        ...
        <div className="space-y-4">
          {filteredCategories.map((category) => (
            <div key={category.id} className="bg-gray-50 rounded-lg p-4 border">
              ...
              {/* แสดงวีดีโอตัวอย่าง 2 เรื่องแรก */}
              <div className="space-y-2">
                {category.videos.slice(0, 2).map((video) => (
                  <div key={video.id} className="flex items-center justify-between p-2 bg-white rounded">
                    <div className="flex-1">
                ...
                {category.videos.length > 2 && (
                  <div className="text-center pt-1">
                    <span className="text-xs text-gray-500">
                      และอีก {category.videos.length - 2} เรื่อง
                    </span>
                  </div>
                )}
              </div>
...
export default HealthRecordVideos; 