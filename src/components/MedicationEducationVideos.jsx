...
const videos = [
  {
    ...
    src: "https://www.youtube.com/embed/CJKZwk4ayZQ",
  },
];

function MedicationEducationVideos({ hideTitle }) {
    ...
    <div className="mt-0">
      {!hideTitle && (
        <h2 className="text-lg font-bold mb-2 text-primary">
          วิดีโอความรู้เรื่องยาสำหรับผู้ป่วย
        </h2>
      )}
      
      {/* คำแนะนำด้านบน */}
      <div className="bg-error/10 border border-error/20 rounded-lg p-4 mb-4">
        <h3 className="font-semibold text-error mb-2">💊 คำแนะนำ</h3>
        <p className="text-sm text-base-content/80 leading-relaxed">
          ความรู้เกี่ยวกับยาที่ใช้ในการรักษาโรคหลอดเลือดสมอง การรับประทานยาที่ถูกต้อง และข้อควรระวัง
        </p>
      </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs text-gray-500">
                  ดูแล้ว {getViewCount(i + 300)} ครั้ง
                </span>
                <button
                  onClick={() => openVideo(v.src.replace('/embed/', '/watch?v='), i + 300)}
...
export default MedicationEducationVideos; 